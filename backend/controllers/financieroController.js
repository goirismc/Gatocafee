...
exports.getPuntoEquilibrio = async (req, res) => {
  try {
    ...
    if (ingresosReales < peIngresos * 0.8) {
      zona = 'perdida';
      mensaje = `⚠️ Zona de pérdida. Necesitas Gs. ${(peIngresos - ingresosReales).toLocaleString()} más para alcanzar el equilibrio.`;
...
exports.getMetas = async (req, res) => {
  try {
    const metas = await MetaMensual.find().sort({ año: -1, mes: -1 }).limit(12);
